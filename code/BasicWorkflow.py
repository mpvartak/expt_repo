...
syncer_obj = Syncer.create_syncer("distribution test", "test_user", \
...
datasets = {
    "train" : Dataset("/path/to/train", {"num_cols" : 15, "dist" : "spiral"}),
    "test" : Dataset("/path/to/test", {"num_cols" : 15, "dist" : "spiral"})
}

model ="model_obj"
mdb_model1 = Model("NN", model, "/path/to/model5")
model_config1 = ModelConfig("NN", {"l1" : 10})
model_metrics1 = ModelMetrics({"accuracy" : 0.54})

mdb_model2 = Model("NN", model, "/path/to/model6")
model_config2 = ModelConfig("NN", {"l1" : 20})
model_metrics2 = ModelMetrics({"accuracy" : 0.44})
